...
function instantiateAddons(parent, project, addonPackages) {
  ...
  // now that we have all the addons, lets make sure all of the before/after
  // directives were properly matched
  let shouldRebuildAddons = false;
  addonNames.forEach(name => {
    addonInfo = addonPackages[name];
    let { before, after } = addonInfo.pkg['ember-addon'];

    // nothing to check here...
    if (!emberAddonConfig.before && !emberAddonConfig.after) { return; }

    if (before) {
      if (typeof before === "string") {
        let addon = findAddonByName(addons, before);
        if (addon && addon.name !== addon.pkg.name) {
          addonInfo.pkg['ember-addon'].before = addon.pkg.name;
          shouldRebuildAddons = true;
        }
      } else {
        for (let i = 0; i < before.length; i++) {
          let addon = findAddonByName(addons, before[i]);
          if (addon && addon.name !== addon.pkg.name) {
            addonInfo.pkg['ember-addon'].before[i] = addon.pkg.name;
            shouldRebuildAddons = true;
          }
        }
      }
    ...
    if (after) {
      if (typeof after === "string") {
        let addon = findAddonByName(addons, after);
        if (addon && addon.name !== addon.pkg.name) {
          addonInfo.pkg['ember-addon'].after = addon.pkg.name;
          shouldRebuildAddons = true;
        }
      } else {
        for (let i = 0; i < after.length; i++) {
          let addon = findAddonByName(addons, after[i]);
          if (addon && addon.name !== addon.pkg.name) {
            addonInfo.pkg['ember-addon'].after[i] = addon.pkg.name;
            shouldRebuildAddons = true;
          }
        }
      }
